/**
 * components/v3-pages/solutions-filtration-v3.tsx
 * V3 production page — /v2/[locale]/solutions/filtration (batch migration).
 * Template parity with solutions-compressors-v3.tsx /
 * solutions-filtration-v3.tsx (same Hero/DataPanel/Evidence/
 * ProcessTimeline/CTA structure, zero component changes). All asset ids
 * and counts are real Industrial Filtration data from assets/asset-library-v2.json,
...
const ROUTE_CAPABILITIES = ["cnc_machining", "surface_treatment", "quality_control"];

/** Real capability-evidence assets (asset-intelligence.json + library pool). */
const CAP_EVIDENCE_ID = "asset-4a71b054";
const CAP_GRID_IDS = ["asset-ddcca769", "asset-0a14eb19", "asset-35ecf959"];
/** One distinct real asset per manufacturing-route step (no in-page reuse). */
const STEP_ASSET_BY_CAPABILITY: Record<string, string> = {"cnc_machining": "asset-42629d01", "surface_treatment": "asset-69853711", "quality_control": "asset-13f80f9d"};
const VERIFY_ASSET_ID = "asset-5767ab83";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-f374417e";

/** Library system_type axis used for the runtime asset count. */
const ASSET_AXIS = "Industrial Filtration";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Industrial Filtration",
    title: "Filtration Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Промышленная фильтрация",
    title: "Маршрут производства фильтрационного оборудования",
    description: (count) =>
...
export default function SolutionsFiltrationV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/filtration");
  ...
  // asset-intelligence.json systems._summary["Industrial Filtration"].
