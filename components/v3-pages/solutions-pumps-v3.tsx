/**
 * components/v3-pages/solutions-pumps-v3.tsx
 * V3 production page — /v2/[locale]/solutions/pumps (migration batch 3).
 * Template parity with solutions-compressors-v3.tsx /
 * solutions-pumps-v3.tsx (same Hero/DataPanel/Evidence/
 * ProcessTimeline/CTA structure, zero component changes). All asset ids
 * and counts are real Pumps & Fluid Handling data from assets/asset-library-v2.json,
...
const ROUTE_CAPABILITIES = ["casting_forging", "cnc_machining", "quality_control"];

/** Real pumps capability-evidence assets (asset-intelligence.json + library trust_evidence pool). */
const CAP_EVIDENCE_ID = "asset-2665de88";
const CAP_GRID_IDS = ["asset-cde61494", "asset-038e0b25", "asset-e77238f5"];
/** One distinct real asset per manufacturing-route step (no in-page reuse). */
const STEP_ASSET_BY_CAPABILITY: Record<string, string> = {"casting_forging": "asset-f1d32059", "cnc_machining": "asset-6897045a", "quality_control": "asset-e1304f61"};
const VERIFY_ASSET_ID = "asset-bcf7cf58";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-74235ce1";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Pumps & Fluid Handling",
    title: "Pump Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Насосы и перекачка жидкостей",
    title: "Маршрут производства насосов",
    description: (count) =>
...
export default function SolutionsPumpsV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/pumps");
  ...
  // asset-intelligence.json systems._summary["Pumps & Fluid Handling"].
