/**
 * components/v3-pages/solutions-automation-control-v3.tsx
 * V3 production page — /v2/[locale]/solutions/automation-control (batch migration).
 * Template parity with solutions-compressors-v3.tsx /
 * solutions-automation-control-v3.tsx (same Hero/DataPanel/Evidence/
 * ProcessTimeline/CTA structure, zero component changes). All asset ids
 * and counts are real Industrial Automation & Control (library axis: Automation Systems) data from assets/asset-library-v2.json,
...
const ROUTE_CAPABILITIES = ["cnc_machining", "assembly_testing", "quality_control"];

/** Real capability-evidence assets (asset-intelligence.json + library pool). */
const CAP_EVIDENCE_ID = "asset-088bd61e";
const CAP_GRID_IDS = ["asset-48481f58", "asset-349bc2f6", "asset-6c6a3bec"];
/** One distinct real asset per manufacturing-route step (no in-page reuse). */
const STEP_ASSET_BY_CAPABILITY: Record<string, string> = {"cnc_machining": "asset-c154188c", "assembly_testing": "asset-645da34b", "quality_control": "asset-4b345ef4"};
const VERIFY_ASSET_ID = "asset-7cf5509e";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-cdedf3f7";

/** Library system_type axis used for the runtime asset count. */
const ASSET_AXIS = "Automation Systems";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Industrial Automation & Control",
    title: "Industrial Automation & Control Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Промышленная автоматизация и управление",
    title: "Маршрут производства систем автоматизации",
    description: (count) =>
...
export default function SolutionsAutomationControlV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/automation-control");
  ...
  // asset-intelligence.json systems._summary["Automation Systems"].
  const systemAssetCount = getAllAssets().filter((a) => a.system_type === ASSET_AXIS).length;
