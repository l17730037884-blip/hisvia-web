/**
 * components/v3-pages/solutions-hydraulic-v3.tsx
 * Second V3 production page — /v2/[locale]/solutions/hydraulic.
 * Migration isolation layer: template parity with
 * solutions-compressors-v3.tsx (same Hero/DataPanel/Evidence/
 * ProcessTimeline/CTA structure, zero component changes), but all
 * asset ids and counts are real Hydraulic Systems data.
 *
 * Real data sources:
 * - system axis:      data/content-v2/system-pages.json (/solutions/hydraulic)
 * - asset count:      assets/asset-library-v2.json (system_type axis) = 81,
 *                     cross-checked with asset-intelligence.json systems._summary
 * - evidence assets:  asset-placement.json + asset-intelligence.json
 *                     (hydraulic hero/capability/verify + trust_evidence pool)
 */
...
const ROUTE_CAPABILITIES = ["cnc_machining", "casting_forging", "surface_treatment"];

/** Real hydraulic capability-evidence assets (asset-intelligence.json + library trust_evidence pool). */
const CAP_EVIDENCE_ID = "asset-c6af8e1a";
const CAP_GRID_IDS = ["asset-af674c9f", "asset-59795634", "asset-e8e7c4af"];
/** One distinct real asset per manufacturing-route step (no in-page reuse). */
const STEP_ASSET_BY_CAPABILITY: Record<string, string> = {
  cnc_machining: "asset-8a0734c1",
  casting_forging: "asset-c0fd314e",
  surface_treatment: "asset-b7c83cb3",
};
const VERIFY_ASSET_ID = "asset-1f615d88";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-25d04982";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Hydraulic Systems",
    title: "Hydraulic Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Гидравлические системы",
    title: "Маршрут производства гидравлики",
    description: (count) =>
...
export default function SolutionsHydraulicV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/hydraulic");
  ...
  // asset-intelligence.json systems._summary["Hydraulic Systems"].
