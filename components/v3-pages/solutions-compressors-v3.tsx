/**
 * components/v3-pages/solutions-compressors-v3.tsx
 * First V3 production page — /v2/[locale]/solutions/compressors.
 * Migration isolation layer: the only V3 consumer; other systems
 * keep V2SolutionsExperience untouched.
 *
 * Phase V3 Design System Alignment:
 * - Visual benchmark: hisvia-v3-prototype.html (dark industrial).
 * - V3 token layer only: styles/v3-tokens.css + lib/design-system-v3/tokens.ts.
 * - Datasheet hero + spec-callout asset provenance (real metadata).
 * - "Industrial System Assets" count computed from the real asset
 *   library (system_type axis, cross-checked with asset-intelligence.json).
 */
...
const ROUTE_CAPABILITIES = ["cnc_machining", "casting_forging", "assembly_testing"];
const CAP_EVIDENCE_ID = "asset-3a31624e";
const VERIFY_ASSET_ID = "asset-b493dcab";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-d5851861";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Air Compressor Systems",
    title: "Air Compressor Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Компрессорные системы",
    title: "Маршрут производства воздушных компрессоров",
    description: (count) =>
...
export default function SolutionsCompressorsV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/compressors");
  if (!system) return null;

  const routeCapabilities = getCapabilities().filter((c) => ROUTE_CAPABILITIES.includes(c.id));
  const capAssets = routeCapabilities
    .map((c) => resolveAsset(c.asset_ids[0]))
  ...
  // Real library count (system_type axis) — 166, cross-checked with
  // asset-intelligence.json systems.compressors.asset数量.
