/**
 * components/v3-pages/solutions-mechanical-transmission-v3.tsx
 * V3 production page — /v2/[locale]/solutions/mechanical-transmission (batch migration).
 * Template parity with solutions-compressors-v3.tsx /
 * solutions-mechanical-transmission-v3.tsx (same Hero/DataPanel/Evidence/
 * ProcessTimeline/CTA structure, zero component changes). All asset ids
 * and counts are real Mechanical Transmission data from assets/asset-library-v2.json,
...
const ROUTE_CAPABILITIES = ["cnc_machining", "casting_forging", "quality_control"];

/** Real capability-evidence assets (asset-intelligence.json + library pool). */
const CAP_EVIDENCE_ID = "asset-2d750c23";
const CAP_GRID_IDS = ["asset-28a10a62", "asset-58e9c0e0", "asset-c62d0b0f"];
/** One distinct real asset per manufacturing-route step (no in-page reuse). */
const STEP_ASSET_BY_CAPABILITY: Record<string, string> = {"cnc_machining": "asset-e1b2004a", "casting_forging": "asset-2661ea6b", "quality_control": "asset-ff6aab7a"};
const VERIFY_ASSET_ID = "asset-47f4ff1d";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-68a3b048";

/** Library system_type axis used for the runtime asset count. */
const ASSET_AXIS = "Mechanical Transmission";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Mechanical Transmission",
    title: "Mechanical Transmission Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Механические передачи",
    title: "Маршрут производства механических передач",
    description: (count) =>
...
export default function SolutionsMechanicalTransmissionV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/mechanical-transmission");
  ...
  // asset-intelligence.json systems._summary["Mechanical Transmission"].
