/**
 * components/v3-pages/solutions-automation-v3.tsx
 * V3 production page — /v2/[locale]/solutions/automation (batch migration).
 * Template parity with solutions-compressors-v3.tsx /
 * solutions-automation-v3.tsx (same Hero/DataPanel/Evidence/
 * ProcessTimeline/CTA structure, zero component changes). All asset ids
 * and counts are real Pneumatic Automation (library axis: Automation Systems) data from assets/asset-library-v2.json,
...
const ROUTE_CAPABILITIES = ["cnc_machining", "assembly_testing", "surface_treatment"];

/** Real capability-evidence assets (asset-intelligence.json + library pool). */
const CAP_EVIDENCE_ID = "asset-aae60e67";
const CAP_GRID_IDS = ["asset-58e52f7c", "asset-f611401c", "asset-88d01697"];
/** One distinct real asset per manufacturing-route step (no in-page reuse). */
const STEP_ASSET_BY_CAPABILITY: Record<string, string> = {"cnc_machining": "asset-3504df7e", "assembly_testing": "asset-8538db43", "surface_treatment": "asset-ed341779"};
const VERIFY_ASSET_ID = "asset-d83d5def";
/** Real library entry whose own metadata forbids hero use — surfaced via spec callout. */
const FORBIDDEN_HERO_ID = "asset-8f49bdc0";
...
const LABELS: Record<Locale, {
  ...
  en: {
    eyebrow: "System · Pneumatic Automation",
    title: "Pneumatic Automation Manufacturing Route",
    description: (count) =>
  ...
  ru: {
    eyebrow: "Система · Пневмоавтоматика",
    title: "Маршрут производства пневмоавтоматики",
    description: (count) =>
...
export default function SolutionsAutomationV3({ locale }: { locale: Locale }) {
  const t = LABELS[locale];
  const system = getSystemPageByRoute("/solutions/automation");
